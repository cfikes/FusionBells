	...
	$timeNow = date("H:i");
		try {
			...
			*/
			...
			//Is Today Special.
			$Special = $fusionBellDB->prepare('SELECT calendar.Schedule,schedules.Time,schedules.Tone,schedules.Zone FROM schedules inner join calendar ON schedules.Schedule = calendar.Schedule WHERE calendar.Date = ? AND schedules.Time >= ? ORDER BY schedules.Time LIMIT 1');
			...
			if ( isset($Tone) && isset($Time) && isset($Schedule) ) {
				...
				$Result = $fusionBellDB->prepare("SELECT ZoneValue FROM zones WHERE ZoneName = ? LIMIT 1");
				$Result->execute(array($Zone));
				...
				$Default = $fusionBellDB->prepare('SELECT settings.SettingValue,schedules.Time,schedules.Tone,schedules.Zone FROM schedules inner join settings ON schedules.Schedule = settings.SettingValue WHERE schedules.Time = ? ORDER BY schedules.Time LIMIT 1');
